...
Player.prototype.steer = function() {
    ...
    if (this.ai) {
        ...
        if (pacman.fleeFrom.scared) {
            ...
            // ─── تطبيق خوارزمية البحث لتوقع أفضل مسار ───
            // استدعاء الدالة اللي هتحسب كل مسارات الأشباح المستقبلية 
            calcAllGhostFutureTiles(0, 1, this.tile, getDirFromEnum(this.dirEnum));
            var potentialRoutes = [];  // المصفوفة اللي هنخزن فيها المسارات المحتملة
            var openDirEnums = getOpenDirEnums(this.tile, this.dirEnum);  // الحصول على الاتجاهات المتاحة للحركة
            
            // اللوب دي بتلف على كل اتجاه متاح قدام باك-مان عشان تقيمه
            for (var index = 0; index < openDirEnums.length; index++) {
                ...
                // هنا بيحصل البحث المحدود بالعمق (Depth-Limited Search - DLS)
                // الدالة getOpenPathDistance هتنادي نفسها بشكل متكرر لحد عمق معين وتجيب تقييم المسار
                ...
                // حساب قيمة الـ Heuristic لكل مسار (علشان يختار المسار ذو القيمة الأعلى)
            ...
            // خوارزمية (Greedy) بتختار المسار صاحب أعلى قيمة تقييم (Value) من ضمن المسارات المحتملة
...
// ─── دالة التقييم (Heuristic Evaluation) ───
// الدالة دي بتدي درجة (Value) لكل مسار، وكل ما الدرجة تزيد، يعني المسار أفضل لباك مان.
var calculateValue = function(potentialRoute, oppositeTunnel) {
    // الدرجة المبدئية = المسافة المقطوعة + عدد النقاط الموجودة في الطريق (كل نقطة بتزود القيمة)
    potentialRoute.value = potentialRoute.distance + potentialRoute.dots / 2;
    
    // لو المسار فيه حباية طاقة (Energizer) والأشباح قريبة جداً، بنزود تقييم المسار ده عشان الباك مان ياكلها وينقذ نفسه
    ...
    // لو الشبح خايف، باك مان بيستهدفه ويزود قيمة المسار اللي هيخليه ياكل الشبح أسرع
    if (potentialRoute.scaredGhostDistance) {
        potentialRoute.value = (80 + (AIDepth - potentialRoute.scaredGhostDistance));
    }

    // لو في فاكهة (Fruit) في الطريق والباك مان بيهرب، بيزود القيمة عشان ياكل الفاكهة في طريقه
    if (potentialRoute.fruit) {
        potentialRoute.value = (100 + (AIDepth - potentialRoute.fruit));
    }

    // لو المسار ده فيه عدد النقط المتبقية عشان يكسب اللعبة، بيدي المسار ده أعلى درجة (500) عشان ينهي اللعبة فوراً
    if (potentialRoute.dots === map.dotsLeft()) {
        potentialRoute.value = 500;
    }

    // بيفضل استخدام الأنفاق (Tunnels) لأنها بتلخبط الأشباح وبتخليهم بطيئين
    if (potentialRoute.containsTunnel) {
        potentialRoute.value += 2;
    }
};
// ─── دالة البحث العميق (Depth-Limited Search) ───
// الدالة دي بتشتغل بشكل تكراري (Recursive) عشان تتوقع المسارات وتتجنب الأشباح لحد مسافة (AIDepth)
var getOpenPathDistance = function(tile, dirEnum, numSteps) {
    calcAllGhostFutureTiles(numSteps, 1, tile, getDirFromEnum(dirEnum));
    var intersectGhost = tileIntersectsGhostPaths(tile, numSteps); // بيتحقق هل المسار ده هيصطدم بشبح ولا لأ
    
    // شرط التوقف: لو وصلنا لأقصى عمق (AIDepth) أو المسار ده هيصطدم بشبح
    if (numSteps > AIDepth || intersectGhost) {
        var scaredGhostDistance = intersectGhost.isScared ? intersectGhost.distance : 0;
        // لو الشبح مش خايف، المسار ده مسدود (مسافته 0 ونقاطه 0)
        return {distance: 0, dots: 0, energizer: 0, scaredGhostDistance: scaredGhostDistance, targetTiles: [tile]};
    }

    var openDirEnums = getOpenDirEnums(tile, dirEnum, true); // بيجيب الاتجاهات اللي ممكن يمشي فيها من المربع ده
    var potentialRoutes = [];

    // التكرار العميق (Recursion): هينادي الدالة تاني لكل مربع قادم قدامه
    for (var index = 0; index < openDirEnums.length; index++) {
        var potentialDirEnum = openDirEnums[index];
        var potentialDirection = getDirFromEnum(potentialDirEnum);
